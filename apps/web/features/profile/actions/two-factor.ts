...
import { generate2FA, enable2FA, disable2FA, check2FAStatus, regenerateBackupCodes } from "../http";
import {
  generate2FASchema,
  enable2FASchema,
  disable2FASchema,
    ...
    if (error instanceof HTTPError) {
      try {
        ...
        return {
          ...
          message:
            errorData.message ||
            "Erro ao gerar código 2FA. Tente novamente.",
        };
      } catch {
        return {
          success: false,
...
export async function check2FAStatusAction(
  ...
  try {
    ...
    return {
      success: true,
      message: response.message || "Status 2FA verificado",
      data: response.data,
    };
  } catch (error) {
...
export async function regenerateBackupCodesAction(
  ...
  try {
    ...
    const response = await regenerateBackupCodes(validatedData.token, authToken);
    ...
    if (error instanceof Error && "errors" in error) {
      ...
      return {
        success: false,
        message:
          (validationError.errors as Array<{message: string}>)?.[0]?.message ||
          "Código inválido. Digite 6-8 caracteres.",
      };
    }
    ...
    return {
      success: false,
      message: "Erro inesperado ao regenerar códigos de backup. Tente novamente.",
    };
  }
}
